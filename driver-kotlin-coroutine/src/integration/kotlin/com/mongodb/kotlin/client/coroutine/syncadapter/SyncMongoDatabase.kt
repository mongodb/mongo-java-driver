...
data class SyncMongoDatabase(val wrapped: MongoDatabase) : JMongoDatabase {
    ...
    override fun <T : Any> runCommand(command: Bson, resultClass: Class<T>): T = runBlocking {
    ...
    override fun <T : Any> runCommand(
        ...
        resultClass: Class<T>
    ): T = runBlocking { wrapped.runCommand(clientSession.unwrapped(), command, readPreference, resultClass) }
    ...
    override fun createCollection(collectionName: String) {
        runBlocking { wrapped.createCollection(collectionName) }
    }

    override fun createCollection(collectionName: String, createCollectionOptions: CreateCollectionOptions) {
        runBlocking { wrapped.createCollection(collectionName, createCollectionOptions) }
    }

    override fun createCollection(clientSession: ClientSession, collectionName: String) {
        runBlocking { wrapped.createCollection(clientSession.unwrapped(), collectionName) }
    }
    ...
    ) = runBlocking { wrapped.createCollection(clientSession.unwrapped(), collectionName, createCollectionOptions) }

    override fun createView(viewName: String, viewOn: String, pipeline: MutableList<out Bson>) = runBlocking {
    ...
    ) = runBlocking { wrapped.createView(viewName, viewOn, pipeline, createViewOptions) }
    ...
    override fun createView(
        ...
        viewOn: String,
        pipeline: MutableList<out Bson>
    ) = runBlocking { wrapped.createView(clientSession.unwrapped(), viewName, viewOn, pipeline) }
