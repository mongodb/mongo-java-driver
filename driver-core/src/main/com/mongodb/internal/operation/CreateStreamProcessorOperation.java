 ...
 */
public final class CreateStreamProcessorOperation implements WriteOperation<Void> {
    private static final String COMMAND_NAME = "createStreamProcessor";
    ...
    public CreateStreamProcessorOperation(final String processorName, final List<BsonDocument> pipeline,
                                          @Nullable final BsonDocument dlq, @Nullable final String streamMetaFieldName,
                                          @Nullable final String tier, @Nullable final Boolean failover) {
    ...
    private BsonDocument getCommand() {
        BsonArray pipelineArray = new BsonArray(pipeline);
        BsonDocument command = new BsonDocument(COMMAND_NAME, new BsonString(processorName))
                .append("pipeline", pipelineArray);

        BsonDocument options = new BsonDocument();
        if (dlq != null) {
            options.append("dlq", dlq);
        }
        if (streamMetaFieldName != null) {
            options.append("streamMetaFieldName", new BsonString(streamMetaFieldName));
        }
        if (tier != null) {
            options.append("tier", new BsonString(tier));
        }
        if (failover != null) {
            options.append("failover", BsonBoolean.valueOf(failover));
        }
        if (!options.isEmpty()) {
