...
public interface Datastore {
    ...
    /**
     * <p> Find all instances by collectionName, and filter property. </p><p> This is the same as: {@code
     * find(clazzOrEntity).filter(property, value); } </p>
     */
    <T, V> Query<T> find(Class<T> clazz, String property, V value);

    /**
     * <p> Find all instances by collectionName, and filter property. </p><p> This is the same as: {@code
     * find(clazzOrEntity).filter(property, value).offset(offset).limit(size); } </p>
     */
    ...
    /**
     * Find the given entities (by id), verifying they are of the correct type; shorthand for {@code find("_id in",
     *ids)}
     */
    ...
    /**
     * updates all entities found with the operations, if nothing is found insert the update as an entity if
     * "createIfMissing" is true; this is an atomic operation per entity
     */
    ...
    /**
     * updates the first entity found with the operations, if nothing is found insert the update as an entity if
     * "createIfMissing" is true; this is an atomic operation per entity
     */
    ...
    /**
     * Ensures (creating if necessary) the index including the field(s) + directions; eg fields = "field1, -field2"
     * ({field1:1, field2:-1})
     */
    ...
    /**
     * Ensures (creating if necessary) the indexes found during class mapping (using {@code @Indexed, @Indexes)}, possibly
     * in the background
     */
