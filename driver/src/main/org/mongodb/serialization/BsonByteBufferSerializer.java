...
/**
 * A simple ByteBuffer serializer which assumes that the ByteBuffer contains valid BSON.  It works by casting the
 * BSONWriter to a BSONBinaryWriter and directly copies the bytes into the underlying output buffer. <p> For
 * deserialization, it does the reverse: Casts the BSONReader to a BSONBinaryReader and copies a single document into a
 * ByteBuffer. </p> <p> This should even be usable as a nested document by adding an instance of it to a
 * PrimitiveSerializers instance. </p>
 */
...
public class BsonByteBufferSerializer implements CollectibleSerializer<ByteBuffer> {
    ...
    public void serialize(final BSONWriter bsonWriter, final ByteBuffer value) {
        ...
        final BSONBinaryWriter bsonBinaryWriter = (BSONBinaryWriter) bsonWriter;
    ...
    public ByteBuffer deserialize(final BSONReader reader) {
        ...
        final BSONBinaryReader bsonBinaryReader = (BSONBinaryReader) reader;

        final InputBuffer buffer = bsonBinaryReader.getBuffer();
        final int size = buffer.readInt32();
        final ByteBuffer retVal = ByteBuffer.allocate(size);
