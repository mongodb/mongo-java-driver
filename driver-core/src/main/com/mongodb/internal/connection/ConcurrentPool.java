...
public class ConcurrentPool<T> implements Pool<T> {
    ...
    private static final class StateAndPermits {
         ...
         * comments in <a href="https://jira.mongodb.org/browse/JAVA-4435">JAVA-4435</a>.</p>
         * <p>
    ...
    private static void lockInterruptiblyUnfair(final ReentrantReadWriteLock.WriteLock lock) {
        throwIfInterrupted();
        lockUnfair(lock);
    }
    ...
    static void lockUnfair(final ReentrantLock lock) {
        if (!lock.tryLock()) {
            // `ReentrantLock.tryLock` is always unfair, but if it does not succeed, fall back to a potentially fair method
            lock.lock();
        }
    }
    ...
    private static void lockUnfair(final ReentrantReadWriteLock.WriteLock lock) {
        if (!lock.tryLock()) {
            // `WriteLock.tryLock` is always unfair, but if it does not succeed, fall back to a potentially fair method
            lock.lock();
        }
    }
