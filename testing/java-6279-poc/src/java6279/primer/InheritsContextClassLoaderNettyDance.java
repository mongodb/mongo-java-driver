 ...
 * driver thread, rather than the driver's own. This variant nulls the CALLING thread's context class loader before construction, the way Netty does, rather than nulling the new thread's field afterwards.</p>
 */
final class InheritsContextClassLoaderNettyDance {
    static {
        Poc.log("%s is being initialized by %s", InheritsContextClassLoaderNettyDance.class.getName(), InheritsContextClassLoaderNettyDance.class.getClassLoader());
        ...
        callingThread.setContextClassLoader(InheritsContextClassLoaderNettyDance.class.getClassLoader());
        try {
            Poc.buildAndStartThreadNettyDance();
        } finally {
            callingThread.setContextClassLoader(previous);
        }
    }

    private InheritsContextClassLoaderNettyDance() {
    }
}
