...
class AsyncGridFSBucketSpecification extends Specification {
    ...
    def 'should download to stream'() {
        ...
        def filesFindIterable = Mock(AsyncFindIterable)
        def chunksFindIterable = Mock(AsyncFindIterable)
        def filesCollection = Mock(AsyncMongoCollection)
        def sizeOfStream = 10
    ...
    def 'should download to stream using #description'() {
        ...
        'using objectId with session'  | new ObjectId()      | Stub(AsyncClientSession)
        'using bsonValue with session' | new BsonString('1') | Stub(AsyncClientSession)
    }
    ...
    def 'should validate the clientSession is not null'() {
        ...
        when:
        gridFSBucket.uploadFromStream((AsyncClientSession) null, filename, Stub(AsyncInputStream), callback)
        then:
        thrown(IllegalArgumentException)
