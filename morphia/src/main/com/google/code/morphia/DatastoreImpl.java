...
//	final protected Morphia morphia;
	...
	public <T> void ensureIndex(final Class<T> clazz, final String name, final IndexFieldDef[] defs, final boolean unique, final boolean dropDupsOnCreate) {
	...
	public <T> void ensureIndex(final Class<T> clazz, final String name, final String fields, final boolean unique, final boolean dropDupsOnCreate, final boolean background) {
		ensureIndex(clazz, name, QueryImpl.parseFieldsString(fields, clazz, mapr, true), unique, dropDupsOnCreate, background, false);
	}
	
	protected <T> void ensureIndex(final Class<T> clazz, final String name, final BasicDBObject fields, final boolean unique, final boolean dropDupsOnCreate, final boolean background, final boolean sparse) {
	...
	public void ensureIndex(final Class clazz, final String name, final IndexFieldDef[] defs, final boolean unique, final boolean dropDupsOnCreate, final boolean background) {
	...
	protected void ensureIndexes(final MappedClass mc, final boolean background, final ArrayList<MappedClass> parentMCs, final ArrayList<MappedField> parentMFs) {
			...
			for(final Annotation ann : idxs) {
				...
				if (idx != null && idx.value() != null && idx.value().length > 0)
					for(final Index index : idx.value()) {
						final BasicDBObject fields = QueryImpl.parseFieldsString(index.value(), mc.getClazz(), mapr, !index.disableValidation());
	...
	public <T, V> Query<T> find(final String kind, final Class<T> clazz, final String property, final V value, final int offset, final int size) {
	...
	private <T> Iterable<Key<T>> insert(final DBCollection dbColl, final Iterable<T> entities, final WriteConcern wc) {
		final ArrayList<DBObject> ents = entities instanceof List ? new ArrayList<DBObject>(((List<T>)entities).size()) : new ArrayList<DBObject>();
	...
	protected <T> Key<T> postSaveGetKey(final T entity, final DBObject dbObj, final DBCollection dbColl, final Map<Object, DBObject> involvedObjects){
	...
	protected <T> WriteResult tryVersionedUpdate(final DBCollection dbColl, final T entity, final DBObject dbObj, final WriteConcern wc, final DB db, final MappedClass mc) {
		WriteResult wr = null;
		...
		if (oldVersion != null && oldVersion > 0) {
			...
			final UpdateResults<T> res = update(  find(dbColl.getName(), (Class<T>) entity.getClass()).filter(Mapper.ID_KEY, idValue).filter(versionKeyName, oldVersion),
											dbObj, 
											false, 
											false, 
											wc);
	...
	public <T> UpdateResults<T> update(final Query<T> query, final UpdateOperations<T> ops, final boolean createIfMissing, final WriteConcern wc) {
	...
	public <T> UpdateResults<T> updateFirst(final Query<T> query, final UpdateOperations<T> ops, final boolean createIfMissing, final WriteConcern wc) {
	...
	@SuppressWarnings("rawtypes")
	private <T> UpdateResults<T> update(final Query<T> query, final UpdateOperations ops, final boolean createIfMissing, final boolean multi, final WriteConcern wc) {
	...
	private <T> UpdateResults<T> update(final Query<T> query, final DBObject u, final boolean createIfMissing, final boolean multi, final WriteConcern wc) {
	...
	@SuppressWarnings("rawtypes")
	public <T> MapreduceResults<T> mapReduce(final MapreduceType type, final Query q, final Class<T> outputType, final MapReduceCommand baseCommand) {
		...
		final MapReduceCommand cmd = new  MapReduceCommand(dbColl, baseCommand.getMap(), baseCommand.getReduce(), baseCommand.getOutputTarget(), outType, qi.getQueryObject());
		...
		final MapreduceResults mrRes = (MapreduceResults) mapr.fromDBObject(MapreduceResults.class, mpo.getRaw(), createCache());
	...
	public void setDefaultWriteConcern(final WriteConcern wc) {defConcern = wc;}
