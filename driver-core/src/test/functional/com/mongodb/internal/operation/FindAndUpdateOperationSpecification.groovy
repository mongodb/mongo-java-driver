...
class FindAndUpdateOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults and passed values'() {
        ...
        def operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, documentCodec, update)
    ...
    def 'should set optional values correctly when using update pipelines'(){
        ...
        when:
        def operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, documentCodec,
    ...
    def 'should update single document'() {
        ...
        operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, documentCodec, update)
                .filter(new BsonDocument('name', new BsonString('Pete')))
                .returnOriginal(false)
    ...
    def 'should update single document when using custom codecs'() {
        ...
        when:
        def update = new BsonDocument('$inc', new BsonDocument('numberOfJobs', new BsonInt32(1)))
        def operation = new FindAndUpdateOperation<Worker>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, workerCodec, update)
                .filter(new BsonDocument('name', new BsonString('Pete')))
        ...
        when:
        update = new BsonDocument('$inc', new BsonDocument('numberOfJobs', new BsonInt32(1)))
        operation = new FindAndUpdateOperation<Worker>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, workerCodec, update)
                .filter(new BsonDocument('name', new BsonString('Pete')))
                .returnOriginal(false)
        returnedDocument = execute(operation, async)
    ...
    def 'should support bypassDocumentValidation'() {
        ...
        def operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), namespace, ACKNOWLEDGED, false, documentCodec, update)
        execute(operation, async)
    ...
    def 'should throw on write concern error'() {
        ...
        when:
        def operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), new WriteConcern(5, 1), false, documentCodec, update)
                .filter(new BsonDocument('name', new BsonString('Pete')))
        execute(operation, async)
        ...
        when:
        operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), new WriteConcern(5, 1), false, documentCodec, update)
    ...
    def 'should support retryable writes'() {
        ...
        when:
        def update = new BsonDocument('$inc', new BsonDocument('numberOfJobs', new BsonInt32(1)))
        def operation = new FindAndUpdateOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, true, documentCodec, update)
                .filter(new BsonDocument('name', new BsonString('Pete')))
