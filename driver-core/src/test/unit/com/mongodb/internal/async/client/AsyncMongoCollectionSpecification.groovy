...
import static com.mongodb.internal.async.client.TestHelper.createIterable
...
class AsyncMongoCollectionSpecification extends Specification {
    ...
    def uuidRepresentation = JAVA_LEGACY
    def collation = Collation.builder().locale('en').build()

    def 'should return the correct name from getName'() {
        given:
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED, true,
                true, readConcern, uuidRepresentation, new TestOperationExecutor([null]))
    ...
    def 'should behave correctly when using withDocumentClass'() {
        ...
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, executor).withDocumentClass(newClass)

        then:
        collection.getDocumentClass() == newClass
        expect collection, isTheSameAs(new AsyncMongoCollectionImpl(namespace, newClass, codecRegistry, readPreference, ACKNOWLEDGED,
    ...
    def 'should behave correctly when using withCodecRegistry'() {
        ...
        expect collection, isTheSameAs(new AsyncMongoCollectionImpl(namespace, Document, collection.getCodecRegistry(), readPreference,
    ...
    def 'should behave correctly when using withReadPreference'() {
        ...
        when:
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, executor).withReadPreference(newReadPreference)

        then:
        collection.getReadPreference() == newReadPreference
        expect collection, isTheSameAs(new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, newReadPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, executor))
    }
    ...
    def 'should behave correctly when using withWriteConcern'() {
        ...
        when:
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, executor).withWriteConcern(newWriteConcern)

        then:
        collection.getWriteConcern() == newWriteConcern
        expect collection, isTheSameAs(new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, newWriteConcern,
                true, true, readConcern, uuidRepresentation, executor))
    }
    ...
    def 'should behave correctly when using withReadConcern'() {
        ...
        when:
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, executor).withReadConcern(newReadConcern)

        then:
        collection.getReadConcern() == newReadConcern
        expect collection, isTheSameAs(new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
    ...
    def 'should create DistinctIterable correctly'() {
        ...
        then:
        expect distinctIterable, isTheSameAs(new AsyncDistinctIterableImpl(session, namespace, Document, String, codecRegistry,
                readPreference, readConcern, executor, 'field', new BsonDocument(), true))
        ...
        then:
        expect distinctIterable, isTheSameAs(new AsyncDistinctIterableImpl(session, namespace, Document, String, codecRegistry,
                readPreference, readConcern, executor, 'field', filter, true))
    ...
    def 'should create FindIterable correctly'() {
        ...
        then:
        expect findIterable, isTheSameAs(new AsyncFindIterableImpl(session, namespace, Document, Document, codecRegistry,
        ...
        then:
        expect findIterable, isTheSameAs(new AsyncFindIterableImpl(session, namespace, Document, BsonDocument, codecRegistry,
                readPreference, readConcern, executor, new BsonDocument(), true))
    ...
    def 'should create AggregateIterable correctly'() {
        ...
        then:
        expect aggregateIterable, isTheSameAs(new AsyncAggregateIterableImpl(session, namespace, Document, Document, codecRegistry,
        ...
        then:
        expect aggregateIterable, isTheSameAs(new AsyncAggregateIterableImpl(session, namespace, Document, BsonDocument, codecRegistry,
                readPreference, readConcern,  ACKNOWLEDGED, executor, [new Document('$match', 1)], AggregationLevel.COLLECTION, true))
    ...
    def 'should create MapReduceIterable correctly'() {
        ...
        then:
        expect mapReduceIterable, isTheSameAs(new AsyncMapReduceIterableImpl(session, namespace, Document, Document, codecRegistry,
        ...
        then:
        expect mapReduceIterable, isTheSameAs(new AsyncMapReduceIterableImpl(session, namespace, Document, BsonDocument, codecRegistry,
                readPreference, readConcern,  ACKNOWLEDGED, executor, 'map', 'reduce'))
    ...
    def 'should validate the insertMany data correctly'() {
        given:
        def collection = new AsyncMongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, uuidRepresentation, Stub(OperationExecutor))
    ...
    def 'should use RenameCollectionOperation correctly'() {
        ...
        where:
        [session, dropTarget] << [[null, Stub(AsyncClientSession)], [true, false]].combinations()
    }
    ...
    def 'should validate the client session correctly'() {
        ...
        when:
        collection.find((AsyncClientSession) null)
        then:
        thrown(IllegalArgumentException)
        ...
        when:
        collection.listIndexes((AsyncClientSession) null)
        then:
        thrown(IllegalArgumentException)
        ...
        when:
        collection.watch((AsyncClientSession) null)
        then:
        thrown(IllegalArgumentException)
    }
