...
class PlainAuthenticator extends SaslAuthenticator {
    PlainAuthenticator(final MongoCredential credential, final Connection connection, final BufferProvider bufferProvider) {
    ...
    protected SaslClient createSaslClient() {
        ...
        try {
            return Sasl.createSaslClient(new String[]{PLAIN.getMechanismName()}, credential.getUserName(),
                    MONGODB_PROTOCOL, getConnection().getServerAddress().getHost(), null, new CallbackHandler() {
                @Override
