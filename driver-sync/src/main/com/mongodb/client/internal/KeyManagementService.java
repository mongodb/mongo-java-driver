...
class KeyManagementService {
    ...
    KeyManagementService(final Map<String, SSLContext> kmsProviderSslContextMap,
            final ProxySettings proxySettings, final int timeoutMillis) {
    ...
    public InputStream stream(final String kmsProvider, final String host, final ByteBuffer message, @Nullable final Timeout operationTimeout) throws IOException {
        ...
        SSLSocket socket = KmsSocketConnector.connect(sslContext, proxySettings, serverAddress, timeoutMillis,
                remainingMillis(operationTimeout));

        // Establishing a proxy tunnel consumes part of the time budget, so re-check before issuing the request rather
        // than sending one whose budget is already spent.
