...
class AggregateOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        when:
        AggregateOperation operation = new AggregateOperation<Document>(CSOT_TIMEOUT, getNamespace(), [], new DocumentCodec())
    ...
    def 'should throw when using invalid hint'() {
        ...
        def operation = new AggregateOperation<Document>(CSOT_TIMEOUT, getNamespace(), [], new DocumentCodec())
                .hint(hint)
    ...
    def 'should throw an exception when using an unsupported ReadConcern'() {
        given:
        def pipeline = [new BsonDocument('$match', new BsonDocument('a', new BsonString('A')))]
        def operation = new AggregateOperation<Document>(CSOT_TIMEOUT, helper.namespace, pipeline, new DocumentCodec())
    ...
    def 'should support collation'() {
        ...
        def operation = new AggregateOperation<BsonDocument>(CSOT_TIMEOUT, namespace, pipeline, new BsonDocumentCodec())
    ...
    def 'should be able to aggregate on a view'() {
        ...
        new CreateViewOperation(CSOT_TIMEOUT, getDatabaseName(), viewName, getCollectionName(), [], WriteConcern.ACKNOWLEDGED)
                .execute(getBinding(getCluster()))

        when:
        AggregateOperation operation = new AggregateOperation<Document>(CSOT_TIMEOUT, viewNamespace, [], new DocumentCodec())
        def batchCursor = execute(operation, async)
        def results = collectCursorResults(batchCursor)*.getString('name')
    ...
    def 'should be able to aggregate with pipeline'() {
        when:
        AggregateOperation operation = new AggregateOperation<Document>(CSOT_TIMEOUT, getNamespace(),
    ...
    def 'should be able to explain an empty pipeline'() {
        given:
        def operation = new AggregateOperation(CSOT_TIMEOUT, getNamespace(), [], new BsonDocumentCodec())
    ...
    def 'should apply comment'() {
        ...
        new CommandReadOperation<>(CSOT_TIMEOUT, getDatabaseName(), new BsonDocument('profile', new BsonInt32(2)),
        ...
        cleanup:
        new CommandReadOperation<>(CSOT_TIMEOUT, getDatabaseName(), new BsonDocument('profile', new BsonInt32(0)),
                new BsonDocumentCodec()).execute(getBinding())
