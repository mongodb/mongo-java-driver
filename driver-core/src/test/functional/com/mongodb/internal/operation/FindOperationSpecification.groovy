...
class FindOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(TIMEOUT_SETTINGS, getNamespace(), decoder)
    ...
    def 'should set optional values correctly'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(TIMEOUT_SETTINGS, getNamespace(), new DocumentCodec())
                .filter(filter)
                .limit(20)
                .skip(30)
                .hint(hint)
    ...
    def 'should apply filter'() {
        ...
        [async, operation] << [
                [true, false],
                [new FindOperation<Document>(TIMEOUT_SETTINGS, getNamespace(), new DocumentCodec())
    ...
    def 'should throw execution timeout exception from execute'() {
        ...
        [async, operation] << [
                [true, false],
                [new FindOperation<Document>(TIMEOUT_SETTINGS_WITH_MAX_TIME, getNamespace(), new DocumentCodec())]
        ].combinations()
    }
    ...
    def 'should read from a secondary'() {
        ...
        def operation = new FindOperation<Document>(TIMEOUT_SETTINGS, getNamespace(), new DocumentCodec())
        def syncBinding = new ClusterBinding(getCluster(), ReadPreference.secondary(), ReadConcern.DEFAULT, getOperationContext())
    ...
    def 'should read from a secondary when hedge is specified'() {
        ...
        def syncBinding = new ClusterBinding(getCluster(), readPreference, ReadConcern.DEFAULT, getOperationContext())
        def asyncBinding = new AsyncClusterBinding(getAsyncCluster(), readPreference, ReadConcern.DEFAULT, getOperationContext())
    ...
    def 'should add allowDiskUse to command if the server version >= 3.2'() {
        ...
        def operation = new FindOperation<Document>(TIMEOUT_SETTINGS, getNamespace(), new DocumentCodec()).allowDiskUse(true)
    ...
    */
    ...
    def 'should support collation'() {
        ...
        def operation = new FindOperation<BsonDocument>(TIMEOUT_SETTINGS, getNamespace(), new BsonDocumentCodec())
