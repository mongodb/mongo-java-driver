...
class AsyncMongoIterableSubscriptionSpecification extends Specification {

    def 'should do nothing until data is requested'() {
        given:
        def mongoIterable = Mock(AsyncMongoIterable)
    ...
    def 'should throw an error if request is less than 1'() {
        given:
        def observer = new TestObserver()
        observe(Stub(AsyncMongoIterable)).subscribe(observer)
