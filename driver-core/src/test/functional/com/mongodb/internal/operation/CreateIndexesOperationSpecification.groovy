...
class CreateIndexesOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should be able to create a geoHaystack indexes'() {
        given:
        def operation = createOperation([new IndexRequest(new BsonDocument('field', new BsonString('geoHaystack')).append('field1', new BsonInt32(1)))
                                                 .bucketSize(10.0)])
    ...
    def 'should throw on write concern error'() {
        given:
        def keys = new BsonDocument('field', new BsonInt32(1))
        def operation = createOperation(null, getNamespace(), [new IndexRequest(keys)], new WriteConcern(5))
    ...
    def createOperation(final ClientSideOperationTimeout clientSideOperationTimeout, final List<IndexRequest> requests) {
        return new CreateIndexesOperation(clientSideOperationTimeout, getNamespace(), requests, null)
    }
