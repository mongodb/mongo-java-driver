...
import com.mongodb.kotlin.client.coroutine.MockitoHelper.deepRefEq
...
class MongoDatabaseTest {
    ...
    fun shouldCallTheUnderlyingRunCommand() {
        ...
        whenever(wrapped.runCommand(clientSession.wrapped, command, primary, BsonDocument::class.java))
            .doReturn(Mono.fromCallable { BsonDocument() })
        whenever(wrapped.runCommand(command, primaryPreferred, BsonDocument::class.java))
            .doReturn(Mono.fromCallable { BsonDocument() })
        whenever(wrapped.runCommand(clientSession.wrapped, command, primaryPreferred, BsonDocument::class.java))
            .doReturn(Mono.fromCallable { BsonDocument() })
        ...
        verify(wrapped, times(1)).runCommand(clientSession.wrapped, command, primaryPreferred, BsonDocument::class.java)
    ...
    fun shouldCallTheUnderlyingDrop() {
        ...
        whenever(wrapped.drop()).doReturn(Mono.empty())
        whenever(wrapped.drop(clientSession.wrapped)).doReturn(Mono.empty())
        ...
        verify(wrapped).drop()
        verify(wrapped).drop(clientSession.wrapped)
        verifyNoMoreInteractions(wrapped)
    }
    ...
    fun shouldCallTheUnderlyingListCollectionNames() {
        ...
        whenever(wrapped.listCollectionNames(clientSession.wrapped)).doReturn(mock())

        mongoDatabase.listCollectionNames()
        mongoDatabase.listCollectionNames(clientSession)

        verify(wrapped).listCollectionNames()
        verify(wrapped).listCollectionNames(clientSession.wrapped)
        verifyNoMoreInteractions(wrapped)
    }
    ...
    fun shouldCallTheUnderlyingCreateCollection() {
        ...
        whenever(wrapped.createCollection(eq(name), deepRefEq(defaultOptions))).doReturn(Mono.empty())
        whenever(wrapped.createCollection(eq(name2), eq(options))).doReturn(Mono.empty())
        whenever(wrapped.createCollection(eq(clientSession.wrapped), eq(name), deepRefEq(defaultOptions)))
            .doReturn(Mono.empty())
        whenever(wrapped.createCollection(eq(clientSession.wrapped), eq(name2), eq(options))).doReturn(Mono.empty())
        ...
        verify(wrapped).createCollection(eq(clientSession.wrapped), eq(name), deepRefEq(defaultOptions))
        verify(wrapped).createCollection(eq(clientSession.wrapped), eq(name2), eq(options))
        verifyNoMoreInteractions(wrapped)
    }
    ...
    fun shouldCallTheUnderlyingCreateView() {
        ...
        whenever(wrapped.createView(eq(viewName), eq(viewOn), eq(pipeline), refEq(defaultOptions)))
            .doReturn(Mono.empty())
        whenever(wrapped.createView(eq(viewName), eq(viewOn), eq(pipeline), eq(options))).doReturn(Mono.empty())
        whenever(
                wrapped.createView(
                    eq(clientSession.wrapped), eq(viewName), eq(viewOn), eq(pipeline), refEq(defaultOptions)))
            .doReturn(Mono.empty())
        whenever(wrapped.createView(eq(clientSession.wrapped), eq(viewName), eq(viewOn), eq(pipeline), eq(options)))
            .doReturn(Mono.empty())
        ...
        verify(wrapped)
            .createView(eq(clientSession.wrapped), eq(viewName), eq(viewOn), eq(pipeline), refEq(defaultOptions))
        verify(wrapped).createView(eq(clientSession.wrapped), eq(viewName), eq(viewOn), eq(pipeline), eq(options))
        verifyNoMoreInteractions(wrapped)
    }
