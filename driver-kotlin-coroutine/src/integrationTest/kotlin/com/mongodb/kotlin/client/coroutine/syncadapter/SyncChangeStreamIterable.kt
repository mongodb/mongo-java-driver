...
data class SyncChangeStreamIterable<T : Any>(val wrapped: ChangeStreamFlow<T>) :
    JChangeStreamIterable<T>, SyncMongoIterable<ChangeStreamDocument<T>>(wrapped) {
    override fun <R : Any> withDocumentClass(clazz: Class<R>): MongoIterable<R> = runBlocking {
    ...
    override fun cursor(): SyncMongoChangeStreamCursor<ChangeStreamDocument<T>> = SyncMongoChangeStreamCursor(wrapped)
