...
class MongoCollectionSpecification extends Specification {
    ...
    def 'should behave correctly when using withDocumentClass'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor).withDocumentClass(newClass)
        ...
        expect collection, isTheSameAs(new MongoCollectionImpl(namespace, newClass, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor))
    }
    ...
    def 'should behave correctly when using withCodecRegistry'() {
        ...
        when:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, UNSPECIFIED, 60000, executor).withCodecRegistry(newCodecRegistry)
        ...
        expect collection, isTheSameAs(new MongoCollectionImpl(namespace, Document, newCodecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, UNSPECIFIED, 60000, executor))

        when:
        collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, STANDARD, 60000, executor).withCodecRegistry(newCodecRegistry)
        ...
        expect collection, isTheSameAs(new MongoCollectionImpl(namespace, Document, collection.getCodecRegistry(), readPreference,
                ACKNOWLEDGED, true, true, readConcern, STANDARD, 60000, executor))
    }
    ...
    def 'should behave correctly when using withReadPreference'() {
        ...
        when:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor).withReadPreference(newReadPreference)
    ...
    def 'should behave correctly when using withWriteConcern'() {
        ...
        when:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor).withWriteConcern(newWriteConcern)
    ...
    def 'should behave correctly when using withReadConcern'() {
        ...
        when:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor).withReadConcern(newReadConcern)
        ...
        expect collection, isTheSameAs(new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, newReadConcern, JAVA_LEGACY, 60000, executor))
    }
    ...
    def 'should behave correctly when using withTimeout'() {
        ...
        when:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor).withTimeout(newTimeout, MILLISECONDS)

        then:
        collection.getTimeout(MILLISECONDS) == newTimeout
        expect collection, isTheSameAs(new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, newTimeout, executor))

        when:
        collection.withTimeout(-1, MILLISECONDS)

        then:
        thrown(IllegalArgumentException)

        when:
        collection.withTimeout(1, null)
    ...
    def 'should use CountOperation correctly with documentCount'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED, true,
                true, readConcern, JAVA_LEGACY, 60000, executor)
        ...
        expect operation, isTheSameAs(new CountDocumentsOperation(CSOT_FACTORY_TIMEOUT, namespace).filter(filter).retryReads(true))
    ...
    def 'should use CountOperation correctly with estimatedDocumentCount'() {
        ...
        then:
        executor.getClientSession() == session
        expect operation, isTheSameAs(new EstimatedDocumentCountOperation(CSOT_FACTORY_TIMEOUT, namespace)
                .retryReads(true))
    ...
    def 'should create DistinctIterable correctly'() {
        ...
        expect distinctIterable, isTheSameAs(new DistinctIterableImpl<>(session, namespace, Document, String,
                codecRegistry, readPreference, readConcern, executor, 'field', new BsonDocument(), true, 60000))
        ...
        then:
        expect distinctIterable, isTheSameAs(new DistinctIterableImpl<>(session, namespace, Document, String,
                codecRegistry, readPreference, readConcern, executor, 'field', filter, true, 60000))
    ...
    def 'should create FindIterable correctly'() {
        ...
        expect findIterable, isTheSameAs(new FindIterableImpl<>(session, namespace, Document, Document, codecRegistry,
                readPreference, readConcern, executor, new BsonDocument(), true, 60000))
        ...
        expect findIterable, isTheSameAs(new FindIterableImpl<>(session, namespace, Document, BsonDocument,
                codecRegistry, readPreference, readConcern, executor, new BsonDocument(), true, 60000))
        ...
        expect findIterable, isTheSameAs(new FindIterableImpl<>(session, namespace, Document, Document,
                codecRegistry, readPreference, readConcern, executor, new Document(), true, 60000))
    ...
    def 'should create AggregateIterable correctly'() {
        ...
        expect aggregateIterable, isTheSameAs(new AggregateIterableImpl<>(session, namespace, Document, Document,
                ...
                AggregationLevel.COLLECTION, true, 60000))
    ...
    def 'should create ChangeStreamIterable correctly'() {
        ...
        expect changeStreamIterable, isTheSameAs(new ChangeStreamIterableImpl<>(session, namespace, codecRegistry,
                readPreference, readConcern, executor, [], Document, ChangeStreamLevel.COLLECTION, true, 60000),
                ['codec'])
        ...
        expect changeStreamIterable, isTheSameAs(new ChangeStreamIterableImpl<>(session, namespace, codecRegistry,
                ...
                ChangeStreamLevel.COLLECTION, true, 60000), ['codec'])
    ...
    def 'should create MapReduceIterable correctly'() {
        ...
        expect mapReduceIterable, isTheSameAs(new MapReduceIterableImpl<>(session, namespace, Document, Document,
                codecRegistry, readPreference, readConcern, ACKNOWLEDGED, executor, 'map', 'reduce', 60000))
    ...
    def 'bulkWrite should use MixedBulkWriteOperation correctly'() {
        ...
        def collection = new MongoCollectionImpl(namespace, BsonDocument, codecRegistry, readPreference, writeConcern,
                retryWrites, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = { boolean ordered, WriteConcern wc, Boolean bypassValidation, List<Bson> filters ->
            new MixedBulkWriteOperation(CSOT_FACTORY_TIMEOUT, namespace, [
    ...
    def 'insertOne should use MixedBulkWriteOperation correctly'() {
        ...
        def expectedOperation = { WriteConcern wc, Boolean bypassDocumentValidation ->
            new MixedBulkWriteOperation(CSOT_FACTORY_TIMEOUT, namespace, [new InsertRequest(new BsonDocument('_id', new BsonInt32(1)))],
    ...
    def 'should validate the insertMany data correctly'() {
        given:
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, Stub(OperationExecutor))
    ...
    def 'deleteOne should use MixedBulkWriteOperation correctly'() {
        ...
        then:
        result.wasAcknowledged() == writeConcern.isAcknowledged()
        expect operation, isTheSameAs(new MixedBulkWriteOperation(CSOT_FACTORY_TIMEOUT, namespace,
    ...
    def 'should use FindOneAndDeleteOperation correctly'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry,  readPreference, ACKNOWLEDGED,
                retryWrites, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = new FindAndDeleteOperation(CSOT_FACTORY_TIMEOUT, namespace, ACKNOWLEDGED, retryWrites,
                new DocumentCodec())
                .filter(new BsonDocument('a', new BsonInt32(1)))
        ...
        when:
        execute(findOneAndDeleteMethod, session, new Document('a', 1),
                new FindOneAndDeleteOptions().projection(new Document('projection', 1)).collation(collation))
    ...
    def 'should use FindOneAndReplaceOperation correctly'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry,  readPreference, writeConcern,
                retryWrites, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = new FindAndReplaceOperation(CSOT_FACTORY_TIMEOUT, namespace, writeConcern, retryWrites, new DocumentCodec(),
        ...
        when:
        execute(findOneAndReplaceMethod, session, new Document('a', 1), new Document('a', 10),
                new FindOneAndReplaceOptions().projection(new Document('projection', 1)).bypassDocumentValidation(false))
    ...
    def 'should use FindAndUpdateOperation correctly'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry,  readPreference, writeConcern,
                retryWrites, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = new FindAndUpdateOperation(CSOT_FACTORY_TIMEOUT, namespace, writeConcern, retryWrites, new DocumentCodec(),
                new BsonDocument('a', new BsonInt32(10))).filter(new BsonDocument('a', new BsonInt32(1)))
    ...
    def 'should use DropCollectionOperation correctly'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = new DropCollectionOperation(CSOT_FACTORY_TIMEOUT, namespace, ACKNOWLEDGED)
    ...
    def 'should use CreateIndexOperations correctly'() {
        ...
        when:
        def expectedOperation = new CreateIndexesOperation(CSOT_FACTORY_TIMEOUT, namespace,
        ...
        when:
        expectedOperation = new CreateIndexesOperation(CSOT_FACTORY_TIMEOUT, namespace,
    ...
    def 'should use ListIndexesOperations correctly'() {
        ...
        then:
        expect operation, isTheSameAs(new ListIndexesOperation(CSOT_FACTORY_TIMEOUT, namespace, new DocumentCodec()).retryReads(true))
        executor.getClientSession() == session
        ...
        then:
        expect operation, isTheSameAs(new ListIndexesOperation(CSOT_FACTORY_TIMEOUT, namespace, new BsonDocumentCodec()).retryReads(true))
        executor.getClientSession() == session
    ...
    def 'should use DropIndexOperation correctly for dropIndex'() {
        ...
        when:
        def expectedOperation = new DropIndexOperation(CSOT_FACTORY_TIMEOUT, namespace, 'indexName', ACKNOWLEDGED)
        ...
        expectedOperation = new DropIndexOperation(CSOT_FACTORY_TIMEOUT, namespace, keys, ACKNOWLEDGED)
    ...
    def 'should use DropIndexOperation correctly for dropIndexes'() {
        ...
        def collection = new MongoCollectionImpl(namespace, Document, codecRegistry, readPreference, ACKNOWLEDGED,
                true, true, readConcern, JAVA_LEGACY, 60000, executor)
        def expectedOperation = new DropIndexOperation(CSOT_FACTORY_TIMEOUT, namespace, '*', ACKNOWLEDGED)
    ...
    def 'should use RenameCollectionOperation correctly'() {
        ...
        def expectedOperation = new RenameCollectionOperation(CSOT_FACTORY_TIMEOUT, namespace, newNamespace, ACKNOWLEDGED)
