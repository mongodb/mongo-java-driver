...
class ListCollectionsFluentSpecification extends Specification {
    ...
    def 'should build the expected listCollectionOperation'() {
        ...
        def listCollectionFluent = new ListCollectionsFluentImpl<Document>('db', Document, codecRegistry, readPreference, executor)
        ...
        listCollectionFluent.filter(new Document('filter', 2)).batchSize(99).maxTime(999, MILLISECONDS).into([])  { result, t -> }
