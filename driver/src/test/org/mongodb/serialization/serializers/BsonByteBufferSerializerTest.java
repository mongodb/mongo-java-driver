...
public class BsonByteBufferSerializerTest extends MongoClientTestBase {
    ...
    public void shouldBeAbleToQueryThenInsert() {
        ...
        final MongoCollection<ByteBuffer> lazyCollection = getCollection(new BsonByteBufferSerializer());
        final List<ByteBuffer> docs = lazyCollection.into(new ArrayList<ByteBuffer>());
        lazyCollection.admin().drop();
        for (final ByteBuffer cur : docs) {
            cur.flip();
        }
        lazyCollection.insert(docs);


        assertEquals(originalDocuments,
                    getCollection().sort(new SortCriteriaDocument("_id", 1)).into(new ArrayList<Document>()));
    }
}
