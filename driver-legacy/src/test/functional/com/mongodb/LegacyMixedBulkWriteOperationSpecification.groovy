...
class LegacyMixedBulkWriteOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should throw IllegalArgumentException for empty list of requests'() {
        when:
        createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, true, null, [])
    ...
    def 'should return correct result for insert'() {
        ...
        def operation = createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, false, null, inserts)
    ...
    def 'should insert a single document'() {
        ...
        def operation = createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, false, null, asList(insert))
    ...
    def 'should execute unacknowledged write'() {
        ...
        def operation = createBulkWriteOperationForInsert(getNamespace(), true, UNACKNOWLEDGED, false, null,
    ...
    def 'should continue on error when continuing on error'() {
        ...
        def operation = createBulkWriteOperationForInsert(getNamespace(), false, ACKNOWLEDGED, false, null, documents)
    ...
    def 'should not continue on error when not continuing on error'() {
        ...
        def operation = createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, false, null, documents)
    ...
    def 'should support retryable writes'() {
        given:
        def insert = new InsertRequest(new BsonDocument('_id', new BsonInt32(1)))
        def operation = createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, true, null, asList(insert))
    ...
    def 'should remove a document'() {
        ...
        def operation = createBulkWriteOperationForDelete(getNamespace(), true, ACKNOWLEDGED, false, null,
    ...
    def 'should replace a single document'() {
        ...
        createBulkWriteOperationForInsert(getNamespace(), true, ACKNOWLEDGED, false, null, asList(insert))
    ...
    def 'should update nothing if no documents match'() {
        ...
        def operation = createBulkWriteOperationForUpdate(getNamespace(), true, ACKNOWLEDGED,
                false, null, asList(new UpdateRequest(new BsonDocument('x', new BsonInt32(1)),
    ...
    def 'when multi is false should update one matching document'() {
        ...
        getCollectionHelper().insertDocuments(new DocumentCodec(),
                new Document('x', 1),
                new Document('x', 1))
        def operation = createBulkWriteOperationForUpdate(getNamespace(), true, ACKNOWLEDGED, false, null,
