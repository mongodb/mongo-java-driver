...
class QueryBatchCursorFunctionalSpecification extends OperationFunctionalSpecification {
    ...
    def 'server cursor should not be null'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 0, 0, new DocumentCodec(), connectionSource)
    ...
    def 'should throw an Exception when going off the end'() {
        ...
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 2, 0, new DocumentCodec(), connectionSource)
        when:
        cursor.next()
        cursor.next()
        cursor.next()
    ...
    def 'test limit exhaustion'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, limit, batchSize, new DocumentCodec(),
    ...
    def 'should block waiting for next batch on a tailable cursor'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 0, 2, new DocumentCodec(),
                connectionSource, connection)
    ...
    def 'test try next with tailable'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 0, 2, new DocumentCodec(), connectionSource)
    ...
    def 'test maxTimeMS'() {
        ...
        def connection = connectionSource.getConnection()
        def maxTimeMS = 10
        def csotFactory = ClientSideOperationTimeouts.create(null, null, maxTimeMS, 0)
    ...
    def 'should kill cursor if limit is reached on initial query'() throws InterruptedException {
        ...
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 5, 0, new DocumentCodec(),
                connectionSource, connection)
    ...
    def 'should kill cursor if limit is reached on get more'() throws InterruptedException {
        ...
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 5, 3, new DocumentCodec(), connectionSource)
    ...
    def 'test limit with get more'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 5, 2, new DocumentCodec(), connectionSource)
    ...
    def 'test limit with large documents'() {
        ...
        when:
        cursor = new QueryBatchCursor<Document>(CSOT_TIMEOUT, firstBatch, 300, 0, new DocumentCodec(), connectionSource)
