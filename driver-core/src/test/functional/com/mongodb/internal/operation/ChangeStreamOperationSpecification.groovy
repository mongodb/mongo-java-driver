...
class ChangeStreamOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        when:
        ChangeStreamOperation operation = new ChangeStreamOperation<Document>(DEFAULT_CSOT_FACTORY, getNamespace(), FullDocument.DEFAULT,
                [], new DocumentCodec())
    ...
    def 'should set optional values correctly'() {
        when:
        ChangeStreamOperation operation = new ChangeStreamOperation<Document>(DEFAULT_CSOT_FACTORY, getNamespace(),
                FullDocument.UPDATE_LOOKUP, [], new DocumentCodec())
                .batchSize(5)
    ...
    def 'should create the expected command'() {
        ...
        def operation = new ChangeStreamOperation<Document>(NO_CSOT_FACTORY, namespace, FullDocument.DEFAULT, pipeline,
                new DocumentCodec(), changeStreamLevel)
                .batchSize(5)
                .collation(defaultCollation)
    ...
    def 'should decode insert to ChangeStreamDocument '() {
        ...
        def pipeline = [BsonDocument.parse('{$match: {operationType: "insert"}}')]
        def operation = new ChangeStreamOperation<BsonDocument>(DEFAULT_CSOT_FACTORY, helper.getNamespace(), FullDocument.DEFAULT, pipeline,
    ...
    def 'should decode update to ChangeStreamDocument '() {
        ...
        def operation = new ChangeStreamOperation<BsonDocument>(DEFAULT_CSOT_FACTORY, helper.getNamespace(), FullDocument.UPDATE_LOOKUP,
                pipeline, createCodec(BsonDocument, fromProviders(new BsonValueCodecProvider(), new ValueCodecProvider())))
    ...
    def 'should decode rename to ChangeStreamDocument '() {
        ...
        def operation = new ChangeStreamOperation<ChangeStreamDocument>(DEFAULT_CSOT_FACTORY, helper.getNamespace(),
                FullDocument.UPDATE_LOOKUP, pipeline,
                createCodec(BsonDocument, fromProviders(new BsonValueCodecProvider(), new ValueCodecProvider())))
    ...
    def 'should set the startAtOperationTime on the sync cursor'() {
        ...
        new ChangeStreamOperation<BsonDocument>(DEFAULT_CSOT_FACTORY, helper.getNamespace(), FullDocument.DEFAULT, [], CODEC)
