...
class FindOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(CSOT_TIMEOUT, getNamespace(), decoder)
    ...
    def 'should set optional values correctly'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(CSOT_TIMEOUT, getNamespace(), new DocumentCodec())
                .filter(filter)
                .limit(20)
                .skip(30)
                .hint(hint)
    ...
    def 'should apply filter'() {
        ...
        [async, operation] << [
                [true, false],
                [new FindOperation<Document>(CSOT_TIMEOUT, getNamespace(), new DocumentCodec())
    ...
    def 'should add read concern to command'() {
        ...
        def operation = new FindOperation<Document>(CSOT_MAX_TIME, getNamespace(), new DocumentCodec())
    ...
    def 'should add allowDiskUse to command if the server version >= 3.2'() {
        ...
        def operation = new FindOperation<Document>(CSOT_NO_TIMEOUT, getNamespace(), new DocumentCodec()).allowDiskUse(true)
    ...
    def 'should call query on Connection with no $query when there are no other meta operators'() {
        given:
        def operation = new FindOperation<Document>(CSOT_NO_TIMEOUT, getNamespace(), new DocumentCodec())
    ...
    def 'should pass tailable and await data flags through'() {
        ...
        where:
        [async, cursorType, expectedClientSideOperationTimeout] << [
                [true, false],
    ...
    def 'should pass miscallaneous flags through'() {
        given:
        def operation = new FindOperation<BsonDocument>(CSOT_TIMEOUT, namespace, new BsonDocumentCodec())
                .noCursorTimeout(true)
                .partial(true)
                .oplogReplay(true)
    ...
    def 'should support collation'() {
        ...
        def operation = new FindOperation<BsonDocument>(CSOT_TIMEOUT, getNamespace(), new BsonDocumentCodec())
