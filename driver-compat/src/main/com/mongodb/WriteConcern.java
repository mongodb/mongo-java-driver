...
public class WriteConcern implements Serializable {
    ...
    public org.mongodb.WriteConcern toNew() {
        if ( _w instanceof Integer )
            return new org.mongodb.WriteConcern((Integer) _w, _wtimeout, _fsync, _j, _continueOnErrorForInsert);
        else if ( _w instanceof String )
            return new org.mongodb.WriteConcern((String) _w, _wtimeout, _fsync, _j, _continueOnErrorForInsert);
        else {
            throw new IllegalStateException("w is not an integer or a string");
        }
    }
