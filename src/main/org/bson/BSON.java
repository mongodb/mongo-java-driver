...
public class BSON {
    ...
    public static List<Transformer> getEncodingHooks(Class<?> c) {
        return unmodifiableList(_encodingHooks.get(c));
    }
    ...
    static Hooks _encodingHooks = new Hooks();
    static Hooks _decodingHooks = new Hooks();
    ...
    public static BSONObject decode(byte[] b) {
        return _staticDecoder.get().readObject(b);
    }
    ...
    static class Hooks {
        private final ClassMap<List<Transformer>> transforms = new ClassMap<List<Transformer>>();
        ...
        public Object apply(Object o) {
            ...
            List<Transformer> list = transforms.get(o.getClass());
            if (list != null)
        ...
        void add(Class<?> c, Transformer t) {
            any = true;
            List<Transformer> l = transforms.get(c);
            if (l == null) {
                l = transforms.putIfAbsent(c, new CopyOnWriteArrayList<Transformer>());
            }
            l.add(t);
        }

        List<Transformer> get(Class<?> c) {
            return transforms.get(c);
        }
        ...
        void remove(Class<?> c, Transformer t) {
            // as we're storing mutable transformer lists in the ClassMap, we can just remove from it directly
            List<Transformer> transforms = get(c);
            if (transforms != null)
                transforms.remove(t);
        }
    }
