...
class CountDocumentsOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        when:
        CountDocumentsOperation operation = new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace())
    ...
    def 'should get the count'() {
        expect:
        execute(new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()), async) == documents.size()
    ...
    def 'should return 0 if no collection'() {
        ...
        then:
        execute(new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()), async) == 0
    ...
    def 'should use limit with the count'() {
        when:
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()).limit(1)
    ...
    def 'should use skip with the count'() {
        when:
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()).skip(documents.size() - 2)
    ...
    def 'should use hint with the count'() {
        ...
        new CreateIndexesOperation(CSOT_TIMEOUT, getNamespace(), [new IndexRequest(indexDefinition).sparse(true)])
                .execute(getBinding())
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()).hint(indexDefinition)
    ...
    def 'should support hints that are bson documents or strings'() {
        expect:
        execute(new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace()).hint(hint), async) == 5
    ...
    def 'should throw with bad hint'() {
        given:
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, getNamespace())
    ...
    def 'should use the ReadBindings readPreference to set slaveOK'() {
        when:
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, helper.namespace)
    ...
    def 'should create the expected aggregation command'() {
        when:
        def filter = new BsonDocument('filter', new BsonInt32(1))
        def operation = new CountDocumentsOperation(CSOT_MAX_TIME, helper.namespace)
    ...
    def 'should throw an exception when using an unsupported Collation'() {
        given:
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, helper.namespace).collation(defaultCollation)
    ...
    def 'should support collation'() {
        ...
        def operation = new CountDocumentsOperation(CSOT_TIMEOUT, namespace)
