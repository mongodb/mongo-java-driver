...
public class JsonWriterTest {
    ...
    public void testDouble() {
        List<TestData<Double>> tests = Arrays.asList(
    ...
    public void testInt64Shell() {
        List<TestData<Long>> tests = Arrays.asList(
                new TestData<Long>(Long.MIN_VALUE, "NumberLong(\"-9223372036854775808\")"),
    ...
    public void testInt64Strict() {
        List<TestData<Long>> tests = Arrays.asList(
                new TestData<Long>(Long.MIN_VALUE, "-9223372036854775808"),
    ...
    public void testBinaryStrict() {
        List<TestData<Binary>> tests = Arrays.asList(
                new TestData<Binary>(new Binary(new byte[0]), "{ \"$binary\" : \"\", \"$type\" : \"0\" }"),
                new TestData<Binary>(new Binary(new byte[]{1}), "{ \"$binary\" : \"AQ==\", \"$type\" : \"0\" }"),
                new TestData<Binary>(new Binary(new byte[]{1, 2}), "{ \"$binary\" : \"AQI=\", \"$type\" : \"0\" }"),
                new TestData<Binary>(new Binary(new byte[]{1, 2, 3}), "{ \"$binary\" : \"AQID\", \"$type\" : \"0\" }"),
                new TestData<Binary>(new Binary((byte) 10, new byte[]{1, 2, 3}), "{ \"$binary\" : \"AQID\", \"$type\" : \"a\" }")
        );
        for (TestData<Binary> cur : tests) {
            stringWriter = new StringWriter();
            writer = new JsonWriter(stringWriter, new JsonWriterSettings(JsonOutputMode.Strict));
            writer.writeStartDocument();
    ...
    public void testBinaryShell() {
        List<TestData<Binary>> tests = Arrays.asList(
                new TestData<Binary>(new Binary(new byte[0]), "new BinData(0, \"\")"),
                ...
                new TestData<Binary>(new Binary((byte) 10, new byte[]{1, 2, 3}), "new BinData(a, \"AQID\")")
        );
        for (TestData<Binary> cur : tests) {
            stringWriter = new StringWriter();
            writer = new JsonWriter(stringWriter, new JsonWriterSettings(JsonOutputMode.Shell));
    ...
    public void testDateTimeStrict() {
        List<TestData<Date>> tests = Arrays.asList(
                new TestData<Date>(new Date(0), "{ \"$date\" : 0 }"),
    ...
    public void testDateTimeShell() {
        List<TestData<Date>> tests = Arrays.asList(
                new TestData<Date>(new Date(0), "ISODate(\"1970-01-01T00:00:00.000Z\")"),
    ...
    public void testDateTimeTenGen() {
        List<TestData<Date>> tests = Arrays.asList(
                new TestData<Date>(new Date(0), "new Date(0)"),
                new TestData<Date>(new Date(Long.MAX_VALUE), "new Date(9223372036854775807)"),
                new TestData<Date>(new Date(Long.MIN_VALUE), "new Date(-9223372036854775808)")
        );
        for (TestData<Date> cur : tests) {
    ...
    public void testRegularExpressionShell() {
        List<TestData<RegularExpression>> tests = Arrays.asList(
                new TestData<RegularExpression>(new RegularExpression(""), "/(?:)/"),
        ...
        for (TestData<RegularExpression> cur : tests) {
            stringWriter = new StringWriter();
            writer = new JsonWriter(stringWriter, new JsonWriterSettings(JsonOutputMode.Shell));
            writer.writeStartDocument();
    ...
    public void testRegularExpressionStrict() {
        List<TestData<RegularExpression>> tests = Arrays.asList(
                new TestData<RegularExpression>(new RegularExpression(""), "{ \"$regex\" : \"\", \"$options\" : \"\" }"),
                new TestData<RegularExpression>(new RegularExpression("a"), "{ \"$regex\" : \"a\", \"$options\" : \"\" }"),
                new TestData<RegularExpression>(new RegularExpression("a/b"), "{ \"$regex\" : \"a/b\", \"$options\" : \"\" }"),
                new TestData<RegularExpression>(new RegularExpression("a\\b"), "{ \"$regex\" : \"a\\\\b\", \"$options\" : \"\" }"),
                new TestData<RegularExpression>(new RegularExpression("a", "i"), "{ \"$regex\" : \"a\", \"$options\" : \"i\" }"),
                new TestData<RegularExpression>(new RegularExpression("a", "m"), "{ \"$regex\" : \"a\", \"$options\" : \"m\" }"),
                new TestData<RegularExpression>(new RegularExpression("a", "x"), "{ \"$regex\" : \"a\", \"$options\" : \"x\" }"),
                new TestData<RegularExpression>(new RegularExpression("a", "s"), "{ \"$regex\" : \"a\", \"$options\" : \"s\" }"),
                new TestData<RegularExpression>(new RegularExpression("a", "imxs"), "{ \"$regex\" : \"a\", \"$options\" : \"imxs\" }")
        );
        for (TestData<RegularExpression> cur : tests) {
            stringWriter = new StringWriter();
            writer = new JsonWriter(stringWriter, new JsonWriterSettings(JsonOutputMode.Strict));
    ...
    public void testString() {
        List<TestData<String>> tests = Arrays.asList(
                new TestData<String>("", "\"\""),
                new TestData<String>(" ", "\" \""),
