...
Each `PropertyModel` includes:
  ...
  * The property name.
  * The document key name, which is the key for the value when serialized to BSON. By default it is the same as the property name.
...
By default all POJOs **must** include a public or protected, empty, no arguments, constructor, or a constructor / public static method 
annotated with the [`Creator`]({{<apiref "org/bson/codecs/pojo/annotations/Creator.html">}}).
...
The following Conventions are available from the [`Conventions`]({{<apiref "org/bson/codecs/pojo/Conventions.html">}}) class:
  ...
  * The [`CLASS_AND_PROPERTY_CONVENTION`]({{<apiref "org/bson/codecs/pojo/Conventions.html#CLASS_AND_PROPERTY_CONVENTION">}}). Sets the 
        ...
        the PropertyModels. Sets the document key if not set to the property name. If the `idProperty` isn't set and there is a 
        property named `_id` or `id` then it will be marked as the `idProperty`.
...
[`org.bson.codecs.pojo.annotations`]({{<apiref "org/bson/codecs/pojo/annotations/package-summary.html">}}) package:
  ...
  * [`Creator`]({{<apiref "org/bson/codecs/pojo/annotations/Creator.html">}}), marks a public constructor or a public static method as
    the creator for new instances of the class. Can be combined with the `Property` annotation to link the parameters with properties.
  ...
  * [`Id`]({{<apiref "org/bson/codecs/pojo/annotations/Id.html">}}), marks a field to be serialized as the `_id` property.
  * [`Property`]({{<apiref "org/bson/codecs/pojo/annotations/Property.html">}}). Allows for an alternative document key 
...
via the `ANNOTATION_CONVENTION` and the `@Creator` annotation. Any parameters for the creator constructor should be annotated with the 
`@Property` annotation. Below is an example of a `Person` POJO that contains final fields that are set via the annotated constructor:
...
public final class Person {
    ...
    @Creator
    public Person(@Property("personId") final String personId, @Property("firstName") final String firstName,
                  @Property("lastName") final String lastName, @Property("address") final Address address) {
        this.pid = personId;
