...
/**
 * A thread-safe variant of {@link Map} in which all mutative operations (the "destructive" operations described by
 * {@link Map} put, remove and so on) are implemented by making a fresh copy of the underlying map.
 * <p/>
 ...
 * Map} interface, add and addAll are not and throw {@link UnsupportedOperationException}.
 * <p/>
 * The actual copy is performed by an abstract {@link #copy(Map)} method. The method is responsible for the underlying
 * Map implementation (for instance a {@link HashMap}, {@link TreeMap}, {@link LinkedHashMap} etc.) and therefore the
 * semantics of what this map will cope with as far as null keys and values, iteration ordering etc. See the note below
 * about suitable candidates for underlying Map implementations
 * <p/>
 * There are supplied implementations for the common j.u.c {@link Map} implementations via the {@link CopyOnWriteMap}
 * static {@link Builder}.
 * <p/>
 ...
 * WeakHashMap} and {@link LinkedHashMap} with access ordering are actually structurally modified by the {@link
 * #get(Object)} method and are therefore not suitable candidates as delegates for this class.
 *
...
abstract class CopyOnWriteMap<K, V> extends AbstractCopyOnWriteMap<K, V, Map<K, V>> {
    ...
    /**
     * Creates a new {@link CopyOnWriteMap} with an underlying {@link LinkedHashMap}. Iterators for this map will be
    ...
    /**
     * Creates a new {@link CopyOnWriteMap} with an underlying {@link LinkedHashMap} using the supplied map as the
     * initial values. Iterators for this map will be return elements in insertion order.
     * <p/>
     * This map has {@link View.Type.STABLE stable} views.
     */
