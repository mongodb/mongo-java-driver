...
public interface ByteBuf extends Closeable {
    ...
    /**
     * Absolute <i>put</i> method&nbsp;&nbsp;<i>(optional operation)</i>. <p/>
    ...
    /**
     * Relative bulk <i>put</i> method&nbsp;&nbsp;<i>(optional operation)</i>. <p/> <p> This method transfers bytes into this buffer from
     * the given source array.  If there are more bytes to be copied from the array than remain in this buffer, that is, if
     * <tt>length</tt>&nbsp;<tt>&gt;</tt>&nbsp;<tt>remaining()</tt>, then no bytes are transferred and a {@link
     * java.nio.BufferOverflowException} is thrown. <p/> <p> Otherwise, this method copies <tt>length</tt> bytes from the given array into
     * this buffer, starting at the given offset in the array and at the current position of this buffer.  The position of this buffer is
     * then incremented by <tt>length</tt>. <p/> <p> In other words, an invocation of this method of the form
     * <tt>dst.put(src,&nbsp;off,&nbsp;len)</tt> has exactly the same effect as the loop <p/>
     * <pre>
     *     for (int i = off; i < off + len; i++)
     *         dst.put(a[i]); </pre>
     *
     * except that it first checks that there is sufficient space in this buffer and it is potentially much more efficient. </p>
     *
     * @param src    The array from which bytes are to be read
     * @param offset The offset within the array of the first byte to be read; must be non-negative and no larger than
     *               <tt>array.length</tt>
     * @param length The number of bytes to be read from the given array; must be non-negative and no larger than <tt>array.length -
     *               offset</tt>
     * @return This buffer
    ...
    /**
     * Relative <i>put</i> method&nbsp;&nbsp;<i>(optional operation)</i>. <p/> <p> Writes the given byte into this buffer at the current
    ...
    /**
     * Flips this buffer.  The limit is set to the current position and then the position is set to zero.  If the mark is defined then it is
     * discarded. <p/> <p> After a sequence of channel-read or <i>put</i> operations, invoke this method to prepare for a sequence of
     * channel-write or relative <i>get</i> operations.  For example: <p/>
     * <blockquote><pre>
     * buf.put(magic);    // Prepend header
     * in.read(buf);      // Read data into rest of buffer
     * buf.flip();        // Flip buffer
     * out.write(buf);    // Write header + data to channel</pre></blockquote>
     * <p/> <p> This method is often used in conjunction with the {@link java.nio.ByteBuffer#compact compact} method when transferring data
    ...
    /**
     * Clears this buffer.  The position is set to zero, the limit is set to the capacity, and the mark is discarded. <p/> <p> Invoke this
     * method before using a sequence of channel-read or <i>put</i> operations to fill this buffer.  For example: <p/>
     * <blockquote><pre>
     * buf.clear();     // Prepare buffer for reading
     * in.read(buf);    // Read data</pre></blockquote>
     * <p/> <p> This method does not actually erase the data in the buffer, but it is named as if it did because it will most often be used
     * in situations in which that might as well be the case. </p>
     *
     * @return This buffer
     */
    ByteBuf clear();
    ...
    /**
     * Relative <i>get</i> method.  Reads the byte at this buffer's current position, and then increments the position. </p>
     *
     ...
     * <p/>
     * <p> This method transfers bytes from this buffer into the given destination array.  An invocation of this method of the form
     ...
     * <p/>
     * <p> Reads the next eight bytes at this buffer's current position, composing them into a long value according to the current byte
     ...
     * <p/>
     * <p> Reads the next four bytes at this buffer's current position, composing them into an int value according to the current byte
     * order, and then increments the position by four.  </p>
     *
     ...
     * larger than the new limit then it is discarded. </p>
     *
