...
class FindOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should have the correct defaults'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(getNamespace(), decoder, null)
    ...
    def 'should set optional values correctly'() {
        ...
        when:
        FindOperation operation = new FindOperation<Document>(getNamespace(), new DocumentCodec(), null)
                .filter(filter)
                .limit(20)
                .skip(30)
                .hint(hint)
    ...
    def 'should query with default values'() {
        ...
        def operation = new FindOperation<Document>(getNamespace(), new DocumentCodec(), null)
    ...
    def 'should apply filter'() {
        ...
        [async, operation] << [
                [true, false],
                [new FindOperation<Document>(getNamespace(), new DocumentCodec(), null)
    ...
    def 'should read from a secondary when hedge is specified'() {
        ...
        def syncBinding = new ClusterBinding(getCluster(), readPreference)
        def asyncBinding = new AsyncClusterBinding(getAsyncCluster(), readPreference)
    ...
    def 'should add allowDiskUse to command if the server version >= 3.2'() {
        ...
        def operation = new FindOperation<Document>(getNamespace(), new DocumentCodec(), null).allowDiskUse(true)
    ...
    def 'should pass tailable and await data flags through'() {
        ...
        then:
        cursor.operationContext.getTimeoutContext().getMaxAwaitTimeMS() == maxAwaitTimeMS
        // should have maxTimeMS override
        cursor.operationContext.getTimeoutContext().runMaxTimeMS { long actualMaxTimeMs ->
            assertEquals(maxTimeMSForCursor, actualMaxTimeMs)
        }
    ...
    def 'should support collation'() {
        ...
        def operation = new FindOperation<BsonDocument>(getNamespace(), new BsonDocumentCodec(), null)
