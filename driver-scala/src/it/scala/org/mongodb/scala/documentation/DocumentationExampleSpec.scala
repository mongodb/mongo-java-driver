...
class DocumentationExampleSpec extends RequiresMongoDBISpec with FuturesSpec {
    ...
    collection
      .insertOne(
        Document(
          ...
          "qty" -> 100,
          "tags" -> Seq("cotton"),
          "size" -> Document("h" -> 28, "w" -> 35.5, "uom" -> "cm")
        )
      )
      .execute()
    // End Example 1
      ...
      .insertMany(
        Seq(
          Document(
            ...
            "qty" -> 25,
            "tags" -> Seq("blank", "red"),
            "size" -> Document("h" -> 14, "w" -> 21, "uom" -> "cm")
          ),
          Document(
            "item" -> "mat",
            "qty" -> 85,
            "tags" -> Seq("gray"),
            "size" -> Document("h" -> 27.9, "w" -> 35.5, "uom" -> "cm")
          ),
          Document(
            "item" -> "mousepad",
            "qty" -> 25,
            "tags" -> Seq("gel", "blue"),
            "size" -> Document("h" -> 19, "w" -> 22.85, "uom" -> "cm")
          )
        )
      )
      .execute()
  ...
  it should "be able to watch" in withCollection { collection =>
    ...
    val pipeline: List[Bson] =
      List(filter(or(Document("{'fullDocument.username': 'alice'}"), in("operationType", List("delete")))))
