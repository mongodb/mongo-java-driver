...
class FindAndReplaceOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'should replace single document'() {
        ...
        when:
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, documentCodec, jordan)
    ...
    def 'should replace single document when using custom codecs'() {
        ...
        when:
        def operation = new FindAndReplaceOperation<Worker>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, workerCodec,
        ...
        operation = new FindAndReplaceOperation<Worker>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, workerCodec, replacement)
                .filter(new BsonDocument('name', new BsonString('Jordan')))
                .returnOriginal(false)
        returnedDocument = execute(operation, async)
    ...
    def 'should throw an exception if replacement contains update operators'() {
        ...
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, false, documentCodec, replacement)
    ...
    def 'should support bypassDocumentValidation'() {
        ...
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), namespace, ACKNOWLEDGED, false, documentCodec, replacement)
        execute(operation, async)
    ...
    def 'should throw on write concern error'() {
        ...
        when:
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), new WriteConcern(5, 1),
    ...
    def 'should create the expected command'() {
        ...
        def operation = new FindAndReplaceOperation<Document>(CSOT_MAX_TIME.get(), getNamespace(), writeConcern, retryWrites, documentCodec, replacement)
    ...
    def 'should support retryable writes'() {
        ...
        when:
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, true, documentCodec, jordan)
                .filter(new BsonDocument('name', new BsonString('Pete')))
    ...
    def 'should retry if the connection initially fails'() {
        ...
        def cannedResult = new BsonDocument('value', new BsonDocumentWrapper(BsonDocument.parse('{}'), new BsonDocumentCodec()))
        def replacement = BsonDocument.parse('{ replacement: 1}')
        def operation = new FindAndReplaceOperation<Document>(CSOT_NO_TIMEOUT.get(), getNamespace(), ACKNOWLEDGED, true, documentCodec, replacement)
        def expectedCommand = new BsonDocument('findAndModify', new BsonString(getNamespace().getCollectionName()))
                .append('update', replacement)
