 ...
 * driver thread, rather than the driver's own. This variant applies the mitigation, so it shows whether nulling the context class loader closes that edge.</p>
 */
final class InheritsContextClassLoaderButNulled {
    static {
        Poc.log("%s is being initialized by %s", InheritsContextClassLoaderButNulled.class.getName(), InheritsContextClassLoaderButNulled.class.getClassLoader());
        ...
        callingThread.setContextClassLoader(InheritsContextClassLoaderButNulled.class.getClassLoader());
        try {
            Poc.buildAndStartThreadInheritingCcl(true);
        } finally {
            callingThread.setContextClassLoader(previous);
        }
    }

    private InheritsContextClassLoaderButNulled() {
    }
}
