 ...
 */
public abstract class AbstractCommandReadOperation<T> implements ReadOperationSimple<T> {
    private final String commandName;
    private final String databaseName;
    private final CommandCreator commandCreator;
    private final Decoder<T> decoder;

    AbstractCommandReadOperation(final String databaseName, final BsonDocument command, final Decoder<T> decoder) {
        this(databaseName, command.getFirstKey(), (operationContext, serverDescription, connectionDescription) -> command, decoder);
    }

    AbstractCommandReadOperation(final String databaseName, final String commandName, final CommandCreator commandCreator,
    ...
    static <T> CommandReadTransformer<T, T> transformer() {
        return (result, source, connection, operationContext) -> result;
    }

    static <T> CommandReadTransformerAsync<T, T> asyncTransformer() {
        return (result, source, connection, operationContext) -> result;
    }
}
