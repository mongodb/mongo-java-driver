...
public class PojoDecodingPerformanceTest {
    ...
    public void outputBaselinePerformanceForPojoWithNoFields() throws Exception {
        PojoCodec<EmptyPojo> pojoCodec = new PojoCodec<EmptyPojo>(Codecs.createDefault(), EmptyPojo.class);
        // 15,042,527 ops per second - faster than an empty document (9,223,774), which is odd....
        EmptyPojo pojoToRead = new EmptyPojo();
        byte[] emptyDocumentAsByteArrayForReader = gatherTestData(pojoToRead, pojoCodec).toByteArray();
    ...
    public void outputPerformanceForAPojoWithASingleIntField() throws Exception {
        ...
        PojoCodec<IntWrapper> pojoCodec = new PojoCodec<IntWrapper>(Codecs.createDefault(), IntWrapper.class);
        ...
        IntWrapper pojoToRead = new IntWrapper(34);
        byte[] documentAsByteArrayForReader = gatherTestData(pojoToRead, pojoCodec).toByteArray();
    ...
    public void outputPerformanceForAPojoWithASingleStringField() throws Exception {
        ...
        PojoCodec<StringWrapper> pojoCodec = new PojoCodec<StringWrapper>(Codecs.createDefault(), StringWrapper.class);
        // 1,568,890 ops per second
        // 2,547,603 ops per second with caching
        StringWrapper documentToRead = new StringWrapper("theValue");
        byte[] documentAsByteArrayForReader = gatherTestData(documentToRead, pojoCodec).toByteArray();
        decodeDocument(NUMBER_OF_TIMES_FOR_WARMUP, documentAsByteArrayForReader, pojoCodec);
    ...
    public void outputPerformanceForListOfPrimitives() throws Exception {
        PojoCodec<ListWrapper> pojoCodec = new PojoCodec<ListWrapper>(Codecs.createDefault(), ListWrapper.class);
        //   226,317 ops per second
        // 1,775,260 ops per second with caching of getDeclaredField
        ListWrapper pojoToRead = new ListWrapper(Arrays.asList(1, 2, 3));
    ...
    public void outputPerformanceForSimpleMap() throws Exception {
        ...
        PojoCodec<MapWrapper> pojoCodec = new PojoCodec<MapWrapper>(Codecs.createDefault(), MapWrapper.class);
        //   163,350 ops per second
        // 1,460,819 ops per second with caching
        MapWrapper pojoToRead = new MapWrapper();
    ...
    public void outputPerformanceForPojoContainingAnotherPojo() throws Exception {
        PojoCodec<PojoWrapper> pojoCodec = new PojoCodec<PojoWrapper>(Codecs.createDefault(), PojoWrapper.class);
        //   804,983 ops per second
        // 1,383,944 ops per second with caching
        PojoWrapper pojoToRead = new PojoWrapper(new StringWrapper("the value"));
    ...
    public void outputPerformanceForPojoContainingTwoPojo() throws Exception {
        PojoCodec<Person> pojoCodec = new PojoCodec<Person>(Codecs.createDefault(), Person.class);
        // 303,042 ops per second
        // 490,159 ops per second not a massive improvement
        Person pojoToRead = new Person(new Address(), new Name());
    ...
    private <T> void decodeDocument(final int timesToRun, final byte[] inputAsByteArray, final PojoCodec<T> pojoCodec) {
        for (int i = 0; i < timesToRun; i++) {
            BSONBinaryReader reader = new BSONBinaryReader(new BasicInputBuffer(new ByteBufNIO(ByteBuffer.wrap(inputAsByteArray))),
                                                                 true);
            try {
