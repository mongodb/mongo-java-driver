...
class AsyncChangeStreamIterableSpecification extends Specification {
    ...
    def 'should build the expected ChangeStreamOperation'() {
        ...
        def changeStreamIterable = new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern, executor,
                pipeline, Document, ChangeStreamLevel.COLLECTION, true)
        ...
        expect operation, isTheSameAs(new ChangeStreamOperation<Document>(namespace, FullDocument.UPDATE_LOOKUP,
                ...
                .startAfter(resumeToken).retryReads(true))
    }
    ...
    def 'should handle exceptions correctly'() {
        ...
        def changeStreamIterable = new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern, executor,
                pipeline, BsonDocument, ChangeStreamLevel.COLLECTION, true)
        ...
        new AsyncChangeStreamIterableImpl(null, namespace, altRegistry, readPreference, readConcern, executor, pipeline, Document,
                ChangeStreamLevel.COLLECTION, true).into([], futureResultCallback)
        futureResultCallback.get()
        ...
        futureResultCallback = new FutureResultCallback<List<BsonDocument>>()
        new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern, executor, [null], Document
                , ChangeStreamLevel.COLLECTION, true).into([], futureResultCallback)
        futureResultCallback.get()
    ...
    def 'should follow the MongoIterable interface as expected'() {
        ...
        def cannedResults = ['{_id: 1}', '{_id: 2}', '{_id: 3}'].collect {
            new ChangeStreamDocument<Document>(OperationType.INSERT, new BsonDocument(), new BsonDocument(), new BsonDocument(),
                    Document.parse(it), new BsonDocument(), null, null, null, null)
        }
        ...
        def mongoIterable = new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern, executor, [],
    ...
    def 'should check variables using notNull'() {
        given:
        def mongoIterable = new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern,
                Stub(OperationExecutor), [new BsonDocument('$match', new BsonInt32(1))], BsonDocument,
                ChangeStreamLevel.COLLECTION, true)
        ...
        when:
        def results = new FutureResultCallback()
        mongoIterable = new AsyncChangeStreamIterableImpl(null, namespace, codecRegistry, readPreference, readConcern,
                Stub(OperationExecutor), [null], BsonDocument, ChangeStreamLevel.COLLECTION, true)
        mongoIterable.into(target, results)
        results.get()
