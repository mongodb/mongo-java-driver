...
Performance benchmarks for the MongoDB Java Driver using JMH and a custom benchmark
framework.
...
- `com.mongodb.benchmark.benchmarks` — Benchmark suite and individual benchmarks (BSON,
  single/multi-doc, bulk operations)
...
- Run benchmarks: `./gradlew :driver-benchmarks:run` or
  `./gradlew :driver-benchmarks:jmh`
- Requires `-Dorg.mongodb.benchmarks.data` and `-Dorg.mongodb.benchmarks.output` system
  properties
