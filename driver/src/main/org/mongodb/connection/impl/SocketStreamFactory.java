...
public class SocketStreamFactory implements StreamFactory {
    ...
    public Stream create(final ServerAddress serverAddress) {
        ...
        if (socketFactory != null) {
            stream = new org.mongodb.connection.impl.SocketStream(serverAddress, settings, socketFactory);
        }
        else if (sslSettings.isEnabled()) {
            stream = new org.mongodb.connection.impl.SocketStream(serverAddress, settings, SSLSocketFactory.getDefault());
        }
        else if (System.getProperty("org.mongodb.useSocket", "false").equals("true")) {
            stream = new org.mongodb.connection.impl.SocketStream(serverAddress, settings, SocketFactory.getDefault());
        }
        else {
